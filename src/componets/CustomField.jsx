...
const CustomField = forwardRef((props, ref) => {
  ...
  const saveNote = async () => {
    ...
    if (note != '' && update) {
      ...
      await window.ZOHO.CRM.API.updateRecord(config)
        .then(function (crdata) {
          ...
          if(cuAttachmentID != ''){
              ZOHO.CRM.API.delinkRelatedRecord({Entity:"Notes",RecordID:currentRecId,RelatedList:"Attachments",RelatedRecordID:cuAttachmentID})
    ...
    if(currentnoteid != "" && selectedFile != null){
      ZOHO.CRM.API.attachFile({Entity:"Notes",RecordID:currentnoteid,File:{Name:selectedFile.name,Content:selectedFile}}).then(function(udata){
  ...
  }, [textareaContent]); // Trigger effect whenever the textarea content changes
  ...
  return (
    <div >
      ...
      <div className={`styled-div ${isExpanded ? 'addfocus' : ''}`} id="parentDiv" contenteditable="false" onClick={toggleExpand} style={{ height: divHeight }}>
        <textarea
          id="title-expandable-textarea"
          style={{ height: 25, border: isExpanded ? 'none' : `${1}px solid #ccc`, display: addTitleState ? 'inline' : 'none' }}
          rows="1"
        ...
        <textarea
          ...
          style={{ paddingTop: 1, height: textareaHeight, border: isExpanded ? 'none' : `${1}px solid #ccc` }}
          rows="1"
          ...
          <div className="flex-container">
            <div className="column1">
              <div>
                ...
                {!selectedFile &&<span className="attachment-icon" onClick={handleFileUpload}>&#128206;</span>}
              </div>
              ...
              {selectedFile && < ><p style={{ display: 'flex', alignItems: 'center' }}>Selected file: {selectedFile.name}&nbsp;<Tooltip title="Remove"
                placement="bottom" arrow> <ClearIcon onClick={clearFile} /></Tooltip> {fileId && <>&nbsp;&nbsp;<CheckCircleRoundedIcon style={{ color: 'green' }} /></>}</p></>}
            </div>
